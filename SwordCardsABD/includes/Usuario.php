...
class Usuario
{
    ...
    public static function buscaUsuario($nombreUsuario)
    {
        ...
        $conn = $app->conexionBd();
        $query = sprintf("SELECT * FROM usuarios WHERE nick = '%s'", $conn->real_escape_string($nombreUsuario));
        $rs = $conn->query($query);
        $result = false;
        if ($rs) {
            if ( $rs->num_rows == 1) {
                ...
                $fila = $rs->fetch_assoc();
                $user = new Usuario($fila['nick'], $fila['pwd'], $fila['rol']);
    ...
    public static function crea($nombreUsuario, $password, $rol)
    {
        ...
        $user = new Usuario($nombreUsuario, self::hashPassword($password), $rol);
    ...
    private static function inserta($usuario)
    {
        $app = Aplicacion::getSingleton();
        $conn = $app->conexionBd();
        $query=sprintf("INSERT INTO usuarios(nick, pwd, rol) VALUES('%s', '%s', '%s')"
    ...
    private static function actualiza($usuario)
    {
        $app = Aplicacion::getSingleton();
        $conn = $app->conexionBd();
        $query=sprintf("UPDATE usuarios U SET nick = '%s', pwd='%s', rol='%s' WHERE U.idUsuario=%i"
